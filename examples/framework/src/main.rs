...
struct Context<'temp_data> {
    temp: &'temp_data TempAllocator<'temp_data>,
...
impl<'temp_data> Context<'temp_data> {
    fn new(temp: &'temp_data TempAllocator<'temp_data>) -> anyhow::Result<Self> {
        let window =
    ...
    fn iterate(&mut self) -> anyhow::Result<()> {
        ...
        self.draw_buffer.push_rect(
            sx::RectShape::new(sx::Rect::from_center_size(logical_size * 0.5, 32.0))
                .with_fill(Some(sx::Fill::new(sx::Rgba8::FUCHSIA)))
