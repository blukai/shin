...
fn map_cursor_shape_to_name(shape: CursorShape) -> &'static CStr {
    match shape {
        ...
        CursorShape::Grabbing => c"grabbinb",
        CursorShape::ColResize => c"col-resize",
        CursorShape::EResize => c"e-resize",
        CursorShape::EwResize => c"ew-resize",
        CursorShape::NResize => c"n-resize",
        CursorShape::NeResize => c"ne-resize",
        CursorShape::NeswResize => c"nesw-resize",
        CursorShape::NsResize => c"ns-resize",
        CursorShape::NwResize => c"nw-resize",
        CursorShape::NwseResize => c"nwse-resize",
        CursorShape::RowResize => c"row-resize",
        CursorShape::SResize => c"s-resize",
        CursorShape::SeResize => c"se-resize",
        CursorShape::SwResize => c"sw-resize",
        CursorShape::WResize => c"w-resize",
    }
}
...
fn map_cursor_shape_to_enum(shape: CursorShape) -> u32 {
    match shape {
        ...
        CursorShape::Grab => wayland::WP_CURSOR_SHAPE_DEVICE_V1_SHAPE_GRAB,
        CursorShape::Grabbing => wayland::WP_CURSOR_SHAPE_DEVICE_V1_SHAPE_GRABBING,
        CursorShape::ColResize => wayland::WP_CURSOR_SHAPE_DEVICE_V1_SHAPE_COL_RESIZE,
        CursorShape::EResize => wayland::WP_CURSOR_SHAPE_DEVICE_V1_SHAPE_E_RESIZE,
        CursorShape::EwResize => wayland::WP_CURSOR_SHAPE_DEVICE_V1_SHAPE_EW_RESIZE,
        CursorShape::NResize => wayland::WP_CURSOR_SHAPE_DEVICE_V1_SHAPE_N_RESIZE,
        CursorShape::NeResize => wayland::WP_CURSOR_SHAPE_DEVICE_V1_SHAPE_NE_RESIZE,
        CursorShape::NeswResize => wayland::WP_CURSOR_SHAPE_DEVICE_V1_SHAPE_NESW_RESIZE,
        CursorShape::NsResize => wayland::WP_CURSOR_SHAPE_DEVICE_V1_SHAPE_NS_RESIZE,
        CursorShape::NwResize => wayland::WP_CURSOR_SHAPE_DEVICE_V1_SHAPE_NW_RESIZE,
        CursorShape::NwseResize => wayland::WP_CURSOR_SHAPE_DEVICE_V1_SHAPE_NWSE_RESIZE,
        CursorShape::RowResize => wayland::WP_CURSOR_SHAPE_DEVICE_V1_SHAPE_ROW_RESIZE,
        CursorShape::SResize => wayland::WP_CURSOR_SHAPE_DEVICE_V1_SHAPE_S_RESIZE,
        ...
        CursorShape::WResize => wayland::WP_CURSOR_SHAPE_DEVICE_V1_SHAPE_W_RESIZE,
    }
}
...
pub struct WaylandBackend {
    ...
    xkb_api_context: Option<xkbcommon::wrap::ApiContext>,
    xkb_keymap_state_handle: Option<xkbcommon::wrap::KeymapStateHandle>,
...
unsafe extern "C" fn handle_wl_keyboard_keymap(
    ...
    if let Some(ks_handle) = this.xkb_keymap_state_handle.take() {
        this.xkb_api_context
            .as_mut()
            .expect("xkb api context")
            .remove_keymap_state(ks_handle);
    };
    ...
    let ac = match this.xkb_api_context.as_mut() {
        Some(ac) => ac,
        None => match xkbcommon::wrap::ApiContext::new() {
    ...
    match ac.create_keymap_state_from_fd(fd, size) {
        Ok(ks_handle) => this.xkb_keymap_state_handle = Some(ks_handle),
        Err(err) => {
...
unsafe extern "C" fn handle_wl_keyboard_key(
    ...
    let (Some(ac), Some(ks_handle)) = (this.xkb_api_context.as_ref(), this.xkb_keymap_state_handle)
    else {
        log::warn!("received keyboard key event, but xkb was not inited");
        return;
    };
    let ks = ac.get_keymap_state(ks_handle);
    ...
    let xkb_sym = unsafe { (ac.api.xkb_state_key_get_one_sym)(ks.state, xkb_key) };
    let utf32 = unsafe { (ac.api.xkb_keysym_to_utf32)(xkb_sym) };
    ...
    match state {
        wayland::WL_KEYBOARD_KEY_STATE_PRESSED => {
            ...
            if let Some(KeyRepeatInfo { rate, delay }) = this.key_repeat_info {
                assert!(!ks.keymap.is_null());
                if unsafe { (ac.api.xkb_keymap_key_repeats)(ks.keymap, xkb_key) } == 1 {
