...
impl WebBackend {
    pub fn new_boxed(attrs: WindowAttrs) -> anyhow::Result<Box<Self>> {
        ...
        unsafe { js_bindings::canvas_set_size(canvas, width as i32, height as i32) };
        let (mut width, mut height) = (0_i32, 0_i32);
        unsafe { js_bindings::canvas_get_size(canvas, &mut width, &mut height) };
        ...
        let webgl2_context = unsafe { js_bindings::canvas_get_context(canvas, c"webgl2".as_ptr()) };
        if webgl2_context.is_nil() {
            return Err(anyhow!("could not get webgl2 context"));
        }
        unsafe {
            js_bindings::gl_clear_color(webgl2_context, 1.0, 0.0, 0.0, 1.0);
            js_bindings::gl_clear(webgl2_context, 0x00004000);
        }
